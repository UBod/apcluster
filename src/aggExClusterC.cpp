...
RcppExport SEXP aggExClusterC(SEXP sR, SEXP KR, SEXP actClustR, SEXP actExemR,
    ...
    for (int j = i + 1; j < K; j++)
    {
      ...
      if(sel.length() > 0)
      {
        ...
        if(ci.length() > 0)
        {
          ...
          exeMat(i,j) = ex;
          objMat(i,j) = (mean(subsetMatrixToVec(s, ex, colInd[intersect(sel,
            IntegerVector(actClust[i]))-1])) + mean(subsetMatrixToVec(s, ex,
            colInd[intersect(sel, IntegerVector(actClust[j]))-1]))) / 2;
        }
        else
        {
  ...
  for (int k = K - 1; k > 0; k--)
  {
    ...
    if (preserveNames && !Rf_isNull(colnames(s)) && Rf_length(colnames(s)) > 0){
    ...
    indexVec = indexVec[indexVec != indexVec[I] & indexVec != indexVec[J]];
      ...
      if(sel.length() > 0)
      {
        ...
        if(ci.length() > 0)
        {
          ...
          objMat(i-1,k-1) = (mean(subsetMatrixToVec(s, ex, 
            colInd[intersect(sel, IntegerVector(actClust[i-1]))-1])) +
